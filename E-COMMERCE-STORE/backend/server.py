...
app = FastAPI(title="E-Commerce API", version="1.3.0")
...
class Order(BaseModel):
    ...
    order_status: str = "Placed" # Changed default to match frontend select options
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
async def login_customer(request: OAuth2PasswordRequestForm = Depends()):
    ...
    if not customer or not Hash.verify(customer["password_hash"], request.password):
...
async def login_admin(request: OAuth2PasswordRequestForm = Depends()):
    # In a real app, you would create an admin user separately, e.g., via a CLI command.
    # For this example, we'll check for a default admin or create one.
...
# ... (Your other admin routes for products, categories, etc. go here)
